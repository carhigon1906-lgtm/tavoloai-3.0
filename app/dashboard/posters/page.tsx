...
export default function PostersPage() {
  ...
  const generatePoster = async () => {
    ...
    if (!cleanTitle) {
      setGenerationError("Escribe una promo antes de generar.")
      setGenerationStatus("")
    ...
    try {
      const response = await fetch("/api/posters/leonardo", {
        ...
        body: JSON.stringify({
          title: cleanTitle,
          subtitle: "Afiche promocional para restaurante",
          cta: "Pide ahora",
          format: "feed-4-5",
          palette: "amber",
        }),
      ...
      setGenerationError("Ocurrió un error de red al generar el afiche.")
      setGenerationStatus("")
      setGeneratedImageUrl("")
    } finally {
      ...
      <div className="mx-auto flex min-h-screen w-full max-w-4xl flex-col px-6 py-10">
        <div className="flex items-center justify-between gap-4">
          <div>
            ...
            <p className={`mt-3 max-w-xl text-sm text-white/65 ${bodyFont.className}`}>
              Prueba simple: escribe una promoción y genera una imagen con Leonardo AI.
            </p>
          </div>
          <Link
            href="/dashboard"
        ...
        <section className="mt-10 rounded-[28px] border border-white/10 bg-white/[0.03] p-6 shadow-[0_20px_80px_rgba(0,0,0,0.35)]">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className={`text-lg text-white ${displayFont.className}`}>Tu promoción</h2>
              <p className={`text-sm text-white/55 ${bodyFont.className}`}>Solo un campo para validar que el flujo funcione.</p>
            </div>
          </div>
          ...
          <div className="mt-6 grid gap-4">
            <label className={`grid gap-2 text-sm text-white/70 ${bodyFont.className}`}>
              ...
              <input
                ...
                placeholder="Ej: Hamburguesa + papas + bebida por $19.900"
          ...
          {!generationError && generationStatus && (
            <div
              className={`mt-4 rounded-2xl border border-emerald-400/20 bg-emerald-500/10 px-4 py-3 text-sm text-emerald-100 ${bodyFont.className}`}
            >
              {generationStatus}
            </div>
          )}
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between gap-3">
                <div>
                  ...
                  <p className={`text-sm text-white/55 ${bodyFont.className}`}>Imagen generada con el texto actual.</p>
                </div>
                <a
