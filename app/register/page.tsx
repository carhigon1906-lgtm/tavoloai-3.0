...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const message = error?.message ?? "Impossibile creare l'account. Riprova."
          ...
          {!created ? (
            <>
              ...
              <p className="text-sm text-slate-300 mb-6">
                Registrati per iniziare a gestire il tuo ristorante con TavoloAI.
              </p>

              <form onSubmit={handleSubmit} className="space-y-4">
                <label className="block">
                  <span className="text-sm font-medium text-slate-200 mb-2 block">Nome e cognome</span>
                  <input
                    name="name"
                    type="text"
                    value={form.name}
                ...
                <label className="block">
                  <span className="text-sm font-medium text-slate-200 mb-2 block">Email</span>
                  <input
                    name="email"
                    type="email"
                    value={form.email}
                ...
                <label className="block">
                  <span className="text-sm font-medium text-slate-200 mb-2 block">Password</span>
                  <input
                    name="password"
                    type="password"
                    value={form.password}
                ...
                <label className="block">
                  <span className="text-sm font-medium text-slate-200 mb-2 block">Nome del locale</span>
                  <input
                    name="business"
                    type="text"
                    value={form.business}
            ...
            <div className="space-y-4">
              ...
              >
                <h2 className="text-lg font-semibold text-white">Benvenuto su TavoloAI</h2>
                <p className="text-sm text-slate-300 mt-1">
                  Il tuo account è stato creato correttamente. Ecco i prossimi passaggi per configurare il locale:
                </p>

                <ul className="mt-3 space-y-2">
                  {[
                    { icon: Upload, text: "Carica il logo del ristorante" },
                    { icon: Globe, text: "Imposta lingua e valuta" },
                    { icon: BookOpen, text: "Crea il primo menu o apri un esempio" },
